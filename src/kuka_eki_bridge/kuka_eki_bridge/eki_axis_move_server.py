"""
eki_axis_move_server.py — Bidirectional TCP server for KUKA XmlAxisMove.

This server:
  - Listens for a KUKA TCP connection on the configured port.
...
from kuka_eki_bridge.axis_move_xml_utils import (
    TcpXmlAxisMoveBuffer,
    parse_axis_move_xml,
)


class EkiAxisMoveServer:
    """
    Bidirectional TCP server for the KUKA XmlAxisMove program.

    The server receives <Robot> XML messages from the KUKA containing
    current axis/position feedback plus status flags (MoveReady, LimitsOK,
    DeltaOK, MoveExecuted), and responds with <Command> XML messages
    containing the latest target from the GUI.

    Usage:
        server = EkiAxisMoveServer(
            host='0.0.0.0',
            port=59153,
            logger=ros_logger,
            on_feedback=callback,
            get_command_xml=provider,
        )
        server.start()
    ...
    def __init__(
        ...
        """
        Initialize the axis move server.
    ...
    def _server_loop(self) -> None:
        ...
        self._info(
            f'Axis move server listening on {self._host}:{self._port}'
        )
        ...
        self._info('Axis move server loop ended.')
    ...
    def start(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f'Cannot start axis move server: {e}'
            ) from e
        ...
        self._thread = threading.Thread(
            ...
            daemon=True,
            name='eki_axis_move_thread',
        )
        self._thread.start()
        self._info('Axis move server started.')

    def stop(self) -> None:
        """Stop the server gracefully."""
        self._info('Stopping axis move server...')
        self._running = False
        ...
        self._info('Axis move server stopped.')
