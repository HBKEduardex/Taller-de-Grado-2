"""
Launch file for the KUKA EKI Axis Move node.

Starts eki_axis_move_node with parameters from config/axis_move.yaml.

This launch file is independent from the existing:
  - eki_server.launch.py
  - axis_stream.launch.py
  - axis_command_loop.launch.py

Usage:
  ros2 launch kuka_eki_bridge axis_move.launch.py
  ros2 launch kuka_eki_bridge axis_move.launch.py port:=59153
  ros2 launch kuka_eki_bridge axis_move.launch.py safe_mode:=false
  ros2 launch kuka_eki_bridge axis_move.launch.py safe_mode:=false allow_motion_commands:=true
"""
...
def generate_launch_description():
    ...
    default_config = os.path.join(
        pkg_share, 'config', 'axis_move.yaml'
    )
